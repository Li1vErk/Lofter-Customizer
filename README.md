...
> 一个为 LOFTER 带来暗色模式、玻璃卡片材质、自定义背景与装饰、关键词 / 用户过滤、评论区增强（只看作者 / 排序）的浏览器美化扩展。纯本地运行，不上传任何数据。
...
<p align="center">
  <!-- 【待补：评论区增强工具行演示 GIF（只看作者 / 从旧到新 / 加载更多评论），文件名 comment-toolbar.gif，传好后取消下行注释即可】
  <img src="assets/screenshots/comment-toolbar.gif" width="720" alt="评论区增强工具行">
  -->
</p>
...
- 面板配色固定蓝紫、不随页面主题色漂移；网页开暗色（手动或跟随系统）时**面板自动变暗**——悬浮按钮打开的面板**第一帧就是暗色**，不会先闪一下亮色再渐变过去，词卡与音效两个弹窗同步适配
- **长说明收进 `?` 折叠**：玻璃、材质这类参数多的分组，正文只留开关与滑杆，原理与取舍点一下 `?` 再看；各项说明均已按「读完会不会改变操作」精简，面板不再被大段解释撑高
- **一致的细滚动条**：面板主滚动区与装饰图列表内滚共用同一套 6px 主题色细滚动条，不再出现系统默认宽条
...
- **页面装饰图**：往页面上贴 PNG / GIF 贴图（看板娘、挂件、表情包……），支持待机动画与点击互动，详见下方「🎨 页面装饰图」
- 内容透明度独立控制，背景与阅读互不干扰

### 🎨 页面装饰图
面板 →「装饰」栏。往页面上贴任意 PNG / GIF 贴图（看板娘、挂件、表情包……），位置大小随意摆，还能加待机动画与点击互动。

**添加与摆放**
- **添加**：「+ 添加装饰图」选择本地 PNG / GIF，可添加多张，各自独立调节
- **摆放**：点栏顶「编辑位置」进入网页内编辑——直接**拖拽**移动、**拖边角缩放**，改完点「退出编辑」；位置随配置保存，下次打开还在原位
- **每张可调**：「显示」开关（临时藏起来，不用删）、**透明度**滑杆
- **层级（覆盖卡片）**：默认贴在博文卡片**下方**、不挡阅读——此时它**只做待机浮动，点击一律穿透给页面**（不响应任何互动，也不会挡住卡片上的按钮和链接）；开启「覆盖卡片」后贴图显示在卡片上方，点击互动才开始生效

**互动效果（每张贴图一个总开关）**
- **默认静止**：新添加的贴图不会自己动——总开关默认「未开启」、面板收起不可展开。想让贴图活起来，先点开总开关
- **开启后**：自动打开「待机动画 · 浮动」与「点击挤压」，贴图立刻和静止图区分开；emoji 粒子、音效、词卡仍保持关闭，要哪个自己开
- **关闭后**：贴图回到完全静止。已配好的子项都会保留，下次开启原样回来（呼吸模式也会记住你上次选的哪种）
- **待机动画**：**浮动**（上下漂浮）或**挤压**（轻微压扁回弹）两种呼吸模式；「更多」里可调幅度与速度（缓慢 / 正常 / 轻快）
- **点击反馈**——在网页上点一下贴图：
  - **挤压回弹**：像果冻一样压扁再弹起，压扁程度与回弹手感（柔和 / 弹性 / 干脆）可调
  - **emoji 粒子**：点击处弹出 emoji 粒子，可自定义 1~3 个（默认 ✨）
  - **音效**：内置音效可选，也可换成自己的音频文件（「更多设置」）
  - **词卡弹窗**：点击弹出气泡台词，台词自己写、可存多条，显示时长可调；**气泡堆叠数**（1~5，默认 1）＝同一时间最多保留几个气泡——1 时新气泡替换旧气泡；调大后多次点击的气泡会同时留在各自点击的位置，超过数量时最早的先消失，每个气泡到时长后各自淡出
- **点击热区**：决定贴图上多大的范围能触发互动（面板里有「?」详解）。100%＝整张贴图都响应；调小后只有贴图中心区域响应，点边缘会像没贴图一样直接点到网页——贴图挡住页面按钮、经常误触发互动时把它调小

**快捷键**：`Ctrl+Shift+H` 一键显示/隐藏全部装饰图，`Alt+Shift+E` 进入/退出编辑位置（可在「其他 › 快捷键」改键）

### 🔲 卡片
- 圆角大小、卡片间距、阴影全局可调
- **悬停轻微放大**：鼠标停在博文卡片上时整卡放大到 102%（默认开；不喜欢或不跟手可在「卡片 › 卡片外观」关掉，只关这一条动画，圆角 / 阴影 / 材质 / 出现动画都不受影响）
...
### 💬 评论区增强
面板 →「功能」栏「评论区增强」组。控件以一行轻量工具条出现在**评论输入框正上方**（宽度与输入框对齐，无外框融入页面），行内控件**默认熄灭、不改变页面原有行为**。

- **只看作者**：点亮后只显示楼主（帖子作者）的评论，再点熄灭即完全还原。楼主识别走分层探测（博客子域 / 评论 iframe 的 referrer / URL 参数 / 区块内博客链接），**认不出来就不猜**——chip 置灰并给出提示，宁可不筛也不误筛
- **从旧到新**：点亮后把已加载的评论按时间反转（最早的在最上面），再点恢复站点原版顺序。实现是纯 CSS 反转，**不改写页面 DOM**，因此不会被站点重渲染冲掉
- **加载更多评论**：仅在「从旧到新」点亮后出现。倒序视图下官方「查看更多」在列表底部，每翻一页都要滚到底再滚回顶，这个键把它省掉——点一下新评论就近出现，页面几乎不动。官方评论加载完会自动隐藏
- **状态不跨帖保留**：切到另一篇帖子会自动复位成熄灭态，避免"进了新帖发现评论怎么这么少"的困惑
- **开关语义**：面板里的「评论工具行」控制**是否注入这条工具条**（默认开）。关掉 = 页面零改动、完全不注入；开着但 chip 熄灭 = 行为与原生完全一致
- **硬限制**：排序与筛选都只作用于**已加载的评论**（LOFTER 评论分页由 iframe 控制），做不到全量筛选
...
### ⚙️ 功能（往页面注入的功能开关）
本栏的功能**一律独立开关**——不会在你升级后悄悄改变页面行为，某个功能不合口味也能单独关掉。多数默认关闭；「评论工具行」默认开（因为行内控件本身默认熄灭，开着也等同于原生行为，关掉则是完全不注入）。
...
- **评论工具行 / 评论区增强**（只看作者 · 从旧到新 · 加载更多评论）：见上方「评论区增强」一节

### 💾 配置导入导出
面板 →「其他」栏「数据」卡。用于**备份、换电脑迁移，或把某几样设置分享给同好**。

- **导出**：勾选要导出的模块（显示 / 装饰 / 卡片 / 导航 / 功能 / 其他）——全选时按钮是「导出全部」，取消勾选后变成「导出 N 个模块」，右侧实时显示「已选 N/6」；旁边的「全不选 / 全选」链接一键切换
- **只分享某几张装饰图**：勾选「装饰」且有图时，导出键下方会出现「要分享的装饰图 全部 ›」，点开可挑具体哪几张（每张会标出带不带词卡 / 粒子 / 音效）
- **导入**：选择 JSON 文件后按模块覆盖——没包含在文件里的模块保持当前值不动，方便只恢复被自己改坏的那一栏；导入前会先弹确认框
- **装饰图可选「覆盖」还是「追加」**：对方已有装饰图时会弹选择——**追加**＝把新的加在后面、自己原有的不动（分享场景用这个）；**覆盖**＝整体替换
- **别人没有你的图片也能用**：装饰图、自定义音效、词卡台词都已**内嵌在文件里**，对方不需要原图；代价是图多时 JSON 会比较大
- **只想传词卡**：装饰卡的「编辑词卡」弹窗里有按行导入 / 一键复制，纯文本、更轻，不用导整份配置
- 导出文件是**纯 JSON 文本**，只保存在你电脑上、**不会上传**；实现走 Blob + `<a download>`，**不申请 `downloads` 权限**，全程无任何网络请求
