...
> 本批主线：① **tag 屏蔽面板全流程**（chip 悬停屏蔽 → 面板管理 → tag 页常驻入口）；② **#9 表情快捷输入
> 真机验收收口**（最近使用 chips 行、过期自动清理、面板磨砂/深膜联动、管理面板「清空最近使用」）；
> ③ **卡片材质总开关 `card.material`**（storage 门禁 + popup「进阶调节」）；④ **字体名状态条**
> （chips 三态 + 点击切换/停用 + 修未加引号 family 名的非法声明）；⑤ popup 排版整轮统一；⑥ 导入
> 白名单补登记 `sidebar`/`comment`（修复导入静默丢右侧栏/评论区设置）。**已验证**：#9 真机验收通过、
> 导入导出往返实测通过。

### 新增

- **tag 屏蔽面板全流程**：tag chip 悬停出「屏蔽」小按钮（不做确认）；popup「官方黑名单」卡加「拉黑用户 / 屏蔽 tag」模式切换，tag 列表渲染与手动输入（`href` 解码、剥 `/new`、`/total` 后缀）；tag 页页头订阅右侧常驻「屏蔽」钮（样式仿原生 `w-vsbtn-3`，插入后实测顶差 `position:relative` 自校准归零）。防两坑：锚点列表区 React 换节点导致孤儿按钮堆积（id 全局唯一 + `isConnected` 检查）；自愈逻辑变自激振荡（页头逐秒拉长，稳定态零写入）
- **表情面板真机验收批次（#9 收口）**：「最近使用」从面板栏目改为输入框下方 chips 行（模拟 App 端，上贴、与输入框左右缘对齐、深膜下白字换肤）；删除表情/删整包后 recent 死条目自动清理（`lcEpValidPrune`，删包撤销不恢复 recent 为已知取舍）；面板去「最近」栏目、打开直进「老福鸽」；开合动画（120ms 竞态防护，`prefers-reduced-motion` 豁免）；面板磨砂 + 深膜联动（锚点卡 `lc-face-dark` → 面板 `lc-dk`）+ 宽 340；管理面板 foot 右侧新增「清空最近使用」（无确认，记录可重建）
- **卡片材质总开关（`card.material`，默认关）**：开启 = 默认点亮暗色半透明底 + 浅色毛玻璃 + 卡面自适应材质。content.js 五处材质读点统一走总门禁；popup 回填迁移（`material === undefined` → 任一子项开即 true）；首次开启且三项都没定制过时一次全亮；「进阶调节」折叠块收纳单项开关与不透明度滑杆（默认收起、总开关关闭时置灰）
- **字体名状态条**：字体名输入框下解析逗号列表成 chips 并标三态——**生效中**（列表第一个本机可用的，与 CSS 逐个回退一致）/ **已装**（点击置顶切换为主字体）/ **未装**（划线，悬停提示「可能没装，也可能该字体只有英文名」）；折叠态常显摘要「生效中：X」。探测用双基线量宽法（**不用 `document.fonts.check`——它对系统字体把不存在的 family 名误判成已装**，华康超明体实测误报）。点「生效中」= 停用自定义字体回系统默认（新状态位 `font.off`，字体名列表保留，再点恢复）；生效中独占一行、其余按输入顺序排列，切换时其他 chip 不跳动。**顺带修一个潜伏 bug**：`activeFontFamily()` 原样拼串，未加引号的 family 名含括号（如 `華康超明體(P)`）是非法 CSS、整条声明被浏览器丢弃——现在逐名加引号（CSS generic 关键字除外）
- **tag 页台头 / 最新最热 tab 栏接入磨砂材质**：收编进 `ensureBubbleCards` 管线（`stack` 标志 → `isolation:isolate` + 内部站点背景透明化清单 + 子面绘制），「包裹台头和 tab 栏的大容器卡片」不再出现；深膜下 tab 白字
- **popup 排版统一**：滑杆行全量统一为「名称｜拖动条｜百分比右对齐」（顺手修掉「内容透明度」label 定宽折行 bug）；进阶调节子开关改「emoji 粒子」式胶囊（点胶囊即开关，热区限胶囊、一行一个）；装饰图卡片滑杆「透明 100」规范为「不透明度 %」（存储格式不变，无迁移）；分割线动态显隐（解释收起时藏、展开时出现正好托住解释；卡片末尾裸开关去悬空线）；「生效范围」改堆叠布局；卡片材质/导航栏/右侧栏/字体名等 hint 再精简一轮（写行为与兼容信息，不描述观感）

### 修复

- **配置导入静默丢数据**：导入白名单（模块表）漏登记 `sidebar`（右侧栏玻璃）与 `comment`（评论区增强）——整份导出文件里有这两个键，导入却被当「无法识别的项」剔除，右侧栏磨砂等设置静默回默认。已按 UI 所在栏目登记（导航/功能模块），按模块导出同样修复。**教训：新增顶层配置段必须同步登记模块表，导出无白名单所以导出方察觉不到**
- **卡片尖角半透明**（浅色磨砂开启时）：卡面是半透明白 + `backdrop-filter`，而尖角是 border 三角——border 三角画不了模糊，背景直接透上来。博文卡与首页发布栏两处尖角改 **clip-path 三角**（真实宽高盒 + 同款背景 + 同款磨砂），与卡面材质连续；深浅膜切换覆盖同步从 `border-right-color` 改 `background`
- 深膜批次：评论区分割线随深膜调暗（`[data-lc-cmt-area]` 子树，类名家族修正）；「查看更多」暗色去精灵图换半透明底圆角
- 「最热」下拉悬停放大一圈：根盒 `background-color` 与 `box-shadow` 两处 `:hover` 整条删除
- popup 双线（`.toggle` 自带 border-bottom 与自加 border-top 叠加）；「生效范围」chip 竖排（`.row label` 90px 定宽后代选择器误伤 chip）；「暗色底不透明度」滑杆 label 挤两行
- 页头 tag 屏蔽钮与订阅键顶差 6px（行盒高于按钮，vertical-align 各值均不齐，改为插入后实测 + relative 平移归零）
- 内联 `!important` 输给 `:not(#id)` 特异性链：tag 屏蔽钮 hover 文字色改内联 `!important`（灰底上不跟随主题色）

### 封存（不修）

- **磨砂开启时悬浮面板导致的博文卡渐变条闪烁**：实锤 Chromium `backdrop-filter` 被原生弹窗逐帧重采样（面板移动即触发）；`will-change` / `translateZ` 层提升等 T1–T5 五种方案全部无效。唯一出路「面板打开时磨砂临时降级」违背不改磨砂前提，搁置
...
> **2026-09-24 续批**：本日工作全部落在**设置面板与装饰交互**上——面板文案再审一轮（长说明收进 `?`、
> 剔除陈述功能的冗余句）、悬浮按钮面板首帧白闪修复（新增 `theme-boot.js`）、装饰栏扁平化改版
> （去 `grp-card` 容器与 `#dec-list` 内滚、三键吸顶工具行）、数据栏改版（去左列标签使 chips 两行
> 排完、导出键合一 + 动态文案）、配置分享三件套（导入「覆盖 / 追加」、装饰按张导出、说明归位）、
> 装饰图默认值与层级语义修正、**互动总开关从「派生」改回显式开关**、暗色卡片圆角跟随用户设置、
> 面板内按 ESC 无法关闭。同日沉淀测试基建：新增用户级技能 `verify-extension-content-script`
> （jsdom 驾驶 content script / popup.js 做行为回归，含「负向对照防真空通过」约定）。
> **本批已推送 GitHub**（`74e8ed7`，14 files / +2709 / −264；`tools/test-*.js` 五个回归文件与
> `theme-boot.js` 首次入库，`tools/debug-*` 本地探针不入库）。
...
- **设置面板文案再审（2026-09-24）**：把各栏「陈述功能」的说明句删掉或折进 `?`——面板正文只留「读完会不会改变操作」的信息，原理、取舍、边界一律收进对应 `?` 折叠段。同批把面板主滚动区与装饰图列表的滚动条统一成 6px 主题色细条，不再有系统默认宽条突兀地插在窄面板里
- **悬浮按钮面板首帧暗色白闪修复（2026-09-24）**：网页开暗色时，从页面悬浮按钮打开的面板**第一帧是亮色**、再渐变过去（工具栏弹窗因未上屏不可见，只有 FAB 面板暴露）。根因是暗色判定发生在 popup.js 的**异步** storage 回调里，晚于首帧绘制。修法：content.js 打开面板时给 iframe src 带 `?lcDark=1`，新增 `theme-boot.js`（**外链**、在 `<head>` 里早于样式）读该参数给 `<html>` 挂 `lc-pre-dark`，CSS 用 `html.lc-pre-dark body { --lc-bg:#1b1b22 }` 抢先落暗色底；popup.js 拿到真实配置后撤标记、交给常规 `body.dark` 接管。**首版写成内联 `<script>` 被 MV3 的 `script-src 'self'` 静默拦死（file:// 与 jsdom 不复现 CSP）——必须外链**；无头逐帧实测首帧浅色像素为 0
- **装饰栏扁平化 + 吸顶工具行（2026-09-24）**：去掉装饰页的 `grp-card` 大容器与 `#dec-list` 的 `max-height:320px + overflow-y:auto`——**后者是「展开互动效果后突然冒出两条滚动条」的根因**（内滚条 + main 主滚条同框），去掉后 main 成为唯一滚动区，展开只是把页面自然撑长。「编辑位置 / 隐藏装饰图」与「＋ 添加装饰图」（原在列表底部）合并成一条 `position:sticky` **吸顶工具行**，滚到哪都够得着；工具行右缘钉一颗**绝对定位**的 `?`（340px 下 flex 排不下四个元素、问号会被挤到第二排）收纳三个键的说明。空态不再整行收起工具行，只藏三个编辑类键
- **装饰卡：胶囊开关 + 文字链接（2026-09-24）**：每张图的「显示 / 覆盖卡片」等开关由勾选框改为 `.lc-pill` 胶囊（开＝主题色、关＝灰），「更多」「编辑词卡 (N)」改为 `.lc-link` 文字链接——**胶囊与链接的视觉层级如实反映「开关 vs 动作」**（此前两者长得像同级键）。旧勾选框全部移除，回归断言口径同步更新
- **数据栏改版（2026-09-24）**：`导出配置`/`选择模块` 两个左列标签去掉（`.row label` 固定 90px 是「chip 一行只挂一颗、卡片被撑到 ~470px」的根源），chips 占满整行后 6 颗只排两行、卡体高度减半。原两个导出按钮**合成一颗实心主按钮**（新增 `.btn.btn-primary`，面板首个实心键）：全选＝「导出全部」、部分＝「导出 N 个模块」、全不选置灰，右侧带「已选 N/6」计数；静态提示 `取消勾选可只导出部分模块` 升级为**动态 `.lc-link`**（全选态「全不选」/ 部分态「全不选 · 全选」/ 全不选态「全选」，链接永远指向用户没在的那个极端，一物两用不新增控件）。导入与导出加分隔线分组，52 字长说明精简后收纳进 `?`
- **配置分享三件套（2026-09-24）**：① **导入装饰图支持「覆盖 / 追加」**——原先导入是整键覆盖，对方导入你的装饰图会清掉自己已有的那些，与分享场景的心理预期不符；双方都有图时弹选择弹窗（「追加」为主按钮），追加时给导入的图**重新编 id** 防撞车，确认框文案随选择变化；② **装饰模块支持按张导出**——勾选「装饰」且有图时出现「要分享的装饰图 全部 ›」，弹窗列缩略图并标注（词卡×N · 粒子 · 音效），支持全选 / 全不选，全选回落为整份快照导出；③ **导入 `?` 说明**补「文件只保存在本地、不会上传；图片、词卡、自定义音效都已内嵌，对方无需原图；图多时文件较大」。**粒度决策依据**：词卡 / emoji 粒子 / 音效不是独立存储键，而是**每张装饰图的属性**（`interactive.dialogues` / `particles` / `soundFile`），故「只分享词卡」拆不开＝分享那张图；词卡另有独立的纯文本通道（词卡弹窗内按行导入 / 复制到剪贴板），不在数据栏重复造。「为什么不需要原图」属实现原理（base64 内嵌），**不往界面写**，界面只讲对用户有用的那层
- **装饰图默认静止 + 层级 × 点击语义（2026-09-24）**：新上传的贴图此前会**默认开启互动**（模板写死 `breathe: true`，而 `enabled` 当时从子项派生，浮动与点击挤压被自动点亮）。改为模板 `breathe: false`（规范化补全的默认仍为 `true`，**已有装饰图一张不动**），新图上传后完全静止，要不要动、要不要互动由用户自己开。同时给全局点击「安检门」加一条：命中的贴图若 `aboveCards` 为假（贴在卡片**下方**）→ **不触发任何点击互动，也不 `stopPropagation`**（这一下点击还给页面，顺带修掉「贴图偷走页面按钮点击」的副作用），待机浮动不受影响。**顺带预防一个回归**：两个容器内 `img.style.zIndex` 各自从容器内序号取值、必然撞号，旧排序会退化成文档顺序、令「重叠处点看得见的上层贴图没反应」——命中顺序改为 `aboveCards` 优先、同层内再按 z-index 降序
- **互动总开关改回显式开关（2026-09-24）**：「互动效果」行右侧恢复「已开启 / 未开启」胶囊（此前取消了独立总键、改由子功能派生）。未开启＝面板收起且展开键禁用（旁边一句灰字「开启后可调」）；开启＝自动打开**浮动呼吸 + 点击挤压**（与静止贴图区分开），emoji 粒子 / 音效 / 词卡仍保持关闭、由用户自行决定。两个实现细节：**呼吸模式尊重历史选择**（之前选过「挤压」，关掉再开仍回到「挤压」，只有从未配置过的图才补默认「浮动」）；**总开关是唯一裁决者**——把子项全关，总开关依然亮着（贴图静止但保留配置，再开任意子项立即生效），旧数据里残留的粒子 / 呼吸开启记录不会再被派生逻辑偷偷点亮
- **暗色模式卡片圆角不跟随用户设置（2026-09-24 修复）**：浅色下调「圆角大小」正常、暗色下没反应。根因是 **v1.0.0 初版遗留的三处「暗色硬编码 12px / 浅色跟随设置」特判**：`ensureBubbleCards` 的首页卡面矩形（`radius: dark ? "12px" : userRadius`）、`#lc-dark-cards` 通用卡面 `::before`（`border-radius: 12px !important`）、`buildCSS` 定时发布页时间卡片。三处统一改为跟随 `settings.card.radius`。**刻意保留的 12px 没动**：`#listEmptyItem` 空状态卡（注释明示不跟随、两模式一致）与草稿页 / 审核中心 / 队列卡（两模式都是固定 12px，不属暗色特判）
- **面板内按 ESC 无法关闭（2026-09-24 修复）**：表现为「面板内没点过任何地方时 ESC 能关，点过之后就关不掉」，且若刚才点的是按钮，按 ESC 还会给它描一圈黑框（`focus-visible` 启发式）。两个症状同根：ESC 监听挂在**宿主页 document** 上，而面板是 iframe——在面板内点过任何位置，焦点就进了 iframe 的文档，**键盘事件只派发给焦点所在文档**，宿主页永远收不到。修法沿用项目自己的跨帧约定（**走 `chrome.storage` 中继，不用 `postMessage`**，避免撞进 LOFTER 页面的 message 处理器）：popup.js 在「作为 FAB 面板 iframe 嵌入」时（`window.parent !== window`）捕获 ESC → 先 `blur()` 焦点元素（消描框）→ 写中继键 `lc_panel_esc`；content.js 的 FAB 模块在 `storage.onChanged` 收到且面板开着 → `closePanel()`。**工具栏弹窗是顶层文档、不转发**（免得关弹窗时顺手把网页上的面板也关了）。同批给四个弹窗（词卡 / 音效 / 装饰图按张选择 / 导入覆盖-追加）加守卫：弹窗开着时 ESC 只关弹窗、不关整个面板
- **测试基建（2026-09-24）**：本轮的面板与行为回归改为 **jsdom 驾驶 popup.js 与 content script**（7 套，维护在工作目录、不入库：文案口径 / 首帧暗色 / 装饰卡交互 / ESC 中继 / 暗色圆角 / 分享导入导出 / 装饰静止与层级点击）；几何与视觉仍交无头 Edge（`--dump-dom` 读注入探针 + CDP）。两条关键经验：① 面板窄宽截图**只能**靠注入 CSS 把容器钉死到设计宽度再裁切（无头有 ~492px 最小视口下限，`--window-size` 压不下去）；② **「X 不触发」类断言必须配正向前置断言、或临时还原旧实现的负向对照**，否则会真空通过——本轮两次靠这条抓到「测试环境缺桩 → 监听根本没注册 → 断言其实没跑」。已沉淀为用户级技能 `verify-extension-content-script`
...
- **装饰图互动语义（2026-09-24 定）**：`interactive.enabled` 为**显式开关**（不再从子项派生），规范化只在字段缺失时补 `false`；新图模板 `breathe: false`（默认完全静止）；「浮动但不响应点击」**只能靠关掉「覆盖卡片」实现**，挤压没有独立开关（跟随互动总开关）
- **层级与点击（2026-09-24 定）**：贴在卡片下层的贴图（`aboveCards` 为假）只做待机浮动，**不响应点击、也不吞掉该次点击**（点击还给页面）；想让它能点就勾「覆盖卡片」。命中顺序 = `aboveCards` 优先，同层内再按 z-index 降序
- **分享粒度（2026-09-24 定）**：按模块 + 装饰图**按张**；词卡 / 粒子 / 音效是每张图的属性，不做字段级细分；词卡的独立通道放在词卡弹窗（纯文本、按行追加）。图片 / 音效以 base64 内嵌，分享无需原图——**不向界面解释编码原理**，只说「已内嵌、对方无需原图、文件可能较大」
- **面板文案口径（2026-09-24 重申）**：hint 必须驱动读者行为变化，而不是复述功能；视觉层级要如实反映逻辑关系（开关＝胶囊、动作＝文字链接）

### 已知限制

- **装饰图位置存的是视口百分比**：贴角 / 居中这类相对构图在任何屏幕上保真，但绝对尺寸随屏幕放大（20% 宽的图在 1366px 屏幕上 273px、3440px 带鱼屏上 688px），并有 900px 上限兜底；多张图并排的构图在屏宽差很大时会散开。像素级一致需「像素坐标 + 锚点」的新模式，复杂度过高，不做
- **`chrome.storage.local` 有 10MB 配额**：装饰图是配额大户（base64 体积约 ×1.33）。对方已有不少装饰图时，再导入一大份可能撞配额——这是导入 `?` 提示「文件可能较大」的由来，也是分享前建议先看体积的原因
- **磨砂卡片悬停时的邻卡渐变带**：信息流卡片开磨砂（浅色/深色磨砂均受影响，纯色/半透明卡不受影响）时，鼠标悬停上一张卡片的操作行文字（热度/评论等悬停放大变主题色的链接），下一张卡片顶部会出现一条固定尺寸的渐变带：悬停瞬间闪现、悬停期间常驻、鼠标移出卡片后消失。经四轮控制台实证（钉合成层 `will-change:transform`、禁悬停投影/滤镜、Paint flashing 反向验证），锁定为 **Chromium 合成器的 backdrop-filter 过期纹理缓存 bug**——悬停引发的重绘只更新脏区，邻卡被缓存的模糊瓦片未重新光栅化（Paint flashing 开启强制逐帧重绘时条带消失即为佐证）。CSS 层面无干净解，**记为已知限制，不修**；候选绕行方案（磨砂档压掉操作行悬停动效 / 膜下垫底色遮盖）留待用户反馈观感不可接受时再评估：信息流卡片开磨砂（浅色/深色磨砂均受影响，纯色/半透明卡不受影响）时，鼠标悬停上一张卡片的操作行文字（热度/评论等悬停放大变主题色的链接），下一张卡片顶部会出现一条固定尺寸的渐变带：悬停瞬间闪现、悬停期间常驻、鼠标移出卡片后消失。经四轮控制台实证（钉合成层 `will-change:transform`、禁悬停投影/滤镜、Paint flashing 反向验证），锁定为 **Chromium 合成器的 backdrop-filter 过期纹理缓存 bug**——悬停引发的重绘只更新脏区，邻卡被缓存的模糊瓦片未重新光栅化（Paint flashing 开启强制逐帧重绘时条带消失即为佐证）。CSS 层面无干净解，**记为已知限制，不修**；候选绕行方案（磨砂档压掉操作行悬停动效 / 膜下垫底色遮盖）留待用户反馈观感不可接受时再评估
